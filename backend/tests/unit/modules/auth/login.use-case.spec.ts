...
describe('LoginUseCase', () => {
  ...
  it('retorna token e refreshToken com credenciais válidas', async () => {
    ...
    expect(result.user.email).toBe('admin@beezpet.com')
  })

  it('reseta tentativas de login após sucesso', async () => {
    const user = await userRepo.findByEmail('admin@beezpet.com')
    user!.recordFailedLogin()
    await userRepo.save(user!)

    await useCase.execute({ email: 'admin@beezpet.com', password: 'senha123' })

    const updated = await userRepo.findByEmail('admin@beezpet.com')
    expect(updated!.loginAttempts).toBe(0)
  })

  it('lança INVALID_CREDENTIALS para senha errada', async () => {
    await expect(
      useCase.execute({ email: 'admin@beezpet.com', password: 'errada99' }),
    ).rejects.toMatchObject({ code: 'INVALID_CREDENTIALS' })
  })

  it('lança INVALID_CREDENTIALS para e-mail inexistente (sem enumerar usuários)', async () => {
    await expect(
      useCase.execute({ email: 'ninguem@beezpet.com', password: 'senha123' }),
    ).rejects.toMatchObject({ code: 'INVALID_CREDENTIALS' })
  })
  ...
  it('lança ACCOUNT_LOCKED quando conta está bloqueada', async () => {
    ...
    await expect(
      useCase.execute({ email: 'admin@beezpet.com', password: 'senha123' }),
    ).rejects.toMatchObject({ code: 'ACCOUNT_LOCKED' })
  })

  it('incrementa loginAttempts a cada senha errada', async () => {
    await useCase.execute({ email: 'admin@beezpet.com', password: 'errada99' }).catch(() => {})
    await useCase.execute({ email: 'admin@beezpet.com', password: 'errada99' }).catch(() => {})
